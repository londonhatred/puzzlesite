...
<head>
  ...
  <title>Stacks Puzzle Prototype</title>
  <style>
    :root { font-family: system-ui, -apple-system, Segoe UI, Roboto, sans-serif; }
    html, body { height: 100%; }
    body { margin: 0; background: #0b0f14; color: #e7eef7; overflow: auto; }
    header{
    ...
    #searchInput:focus{ border-color: rgba(140,198,255,0.9); box-shadow: 0 0 0 2px rgba(140,198,255,0.18); }
    #clearSearchBtn{
      width: 34px;
      height: 34px;
      border-radius: 12px;
    ...
    button:hover{ background: rgba(231,238,247,0.14); }
    button:disabled{
      opacity: 0.45;
      cursor: not-allowed;
    ...
    button:disabled:hover{ background: rgba(231,238,247,0.06); }
    ...
    .textInput:focus{ border-color: rgba(140,198,255,0.9); box-shadow: 0 0 0 2px rgba(140,198,255,0.18); }
    ...
    .tile.selected{
      border-color: rgba(140,198,255,0.9);
      box-shadow: 0 0 0 2px rgba(140,198,255,0.25);
...
body.modal-open #finishOverlay{ pointer-events: auto; }
    .modal{
      width: min(640px, 100%);
...
async function loadStacksFromSupabaseDaily() {
  ...
  if (eligibleCats.length === 0) return { ok: false, reason: "no eligible categories" };
...
try {
  ...
  if (!boardErr && boardRow && Array.isArray(boardRow.category_ids) && Array.isArray(boardRow.item_ids)) {
    ...
    if (nextSets.length && nextItems.length) {
      ...
      return { ok: true, signature: sig, dayKey };
    }
  }
} catch {
...
const shuffledCats = shuffleOnce(eligibleCats);
  ...
  for (const c of shuffledCats) {
    if (nextSets.length >= Math.min(DAILY_CATEGORY_COUNT, eligibleCats.length)) break;
  ...
  .map(x => ({
    ...
    // Filter out norms already used today
    const cleanPool = pool.filter(p => p.norm && !usedNorms.has(p.norm));
    ...
    if (msg.includes("duplicate") || msg.includes("23505")) {
      ...
      if (boardRow2 && Array.isArray(boardRow2.category_ids) && Array.isArray(boardRow2.item_ids)) {
        ...
        if (storedSets.length && storedItems.length) {
          ...
          return { ok: true, signature: sig2, dayKey };
        }
      }
    } else {
...
const STACKS_DAILY_CACHE_KEY = "stacks_daily_cache_v1";
...
// If supabaseClient is configured, we read/write a shared table `leaderboard_runs`.
// Otherwise we fall back to localStorage only.
...
function getPlayerName() {
  const n = localStorage.getItem("puzzle_player_name_v1") || "";
...
function setBoardSourceLabel(src) {
  stacksBoardSource = src || "demo";
  const el = document.getElementById("boardSource");
  if (el) el.textContent = String(stacksBoardSource).toUpperCase();
}
  ...
  function startTicking() {
    ...
    timerInterval = setInterval(() => {
      ...
      setBoardSourceLabel(stacksBoardSource);
    }, 250);
  }
...
function freezeTimer() {
  ...
  function setStats() {
    ...
    document.getElementById("completedSets").textContent = String(state.completedSets);
  ...
  function isSolved() {
    return state.completedSets >= sets.length;
  }
  ...
  // Submit to local leaderboard once per completed puzzle
  ...
  // Show finish popup once per puzzle until dismissed
if (localStorage.getItem(FINISH_DISMISSED_KEY) !== "1") {
  showFinishModal();
}
} else {
  saveState();
...
try {
  await ensureStacksBoardReady();
    ...
    state = freshState();
    clearSavedState();
    localStorage.removeItem(FINISH_DISMISSED_KEY);
    saveState();
    render();
...
const FINISH_DISMISSED_KEY = "stacks_finish_dismissed_v1";
...
function showFinishModal() {
  ...
  const shown = state.elapsedMs + (state.timerRunning && state.startMs ? (Date.now() - state.startMs) : 0);
  ...
  finishBody.innerHTML = `
    ...
    <p><strong>Mistakes:</strong> ${escapeHtml(String(state.mistakes))} &nbsp; <strong>Correct links:</strong> ${escapeHtml(String(state.correctLinks))}</p>
    <p><strong>New board in:</strong> <span id="finishNextBoardIn" class="mono">${escapeHtml(nextIn)}</span></p>
    <p class="muted">There'll be a share function later, but for now, screenshot and gloat to your pals.</p>
  `;
...
function closeFinishModal() {
  ...
  stopFinishCountdown();
  try { localStorage.setItem(FINISH_DISMISSED_KEY, "1"); } catch {}
}
...
if (res && res.ok && res.signature) supaSig = res.signature;
    ...
    if (isSolved() && localStorage.getItem(FINISH_DISMISSED_KEY) !== "1") {
  showFinishModal();
}
...
function stopNextBoardCountdown() {
  ...
  function showView(which) {
    ...
    if (which === "stacks") {
      // Show how-to on first ever entry; Start Game begins timer immediately
      maybeShowHowToFor("stacks");
      if (localStorage.getItem(howToKey("stacks")) === "1") {
        // Do not auto-start timer on every entry; only start if never started yet
        // (Timer starts on first click, but Start Game from modal forces it.)
      }
    }
    currentView = which;
  }
